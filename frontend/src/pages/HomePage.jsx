...
const stats = [
  { value: '10 000+', label: 'Волонтёров', icon: '🧑‍🤝‍🧑' },
  { value: '20', label: 'Команд', icon: '🏆' },
  { value: '500+', label: 'Мероприятий', icon: '📅' },
  { value: '100+', label: 'Портфолио', icon: '📄' },
]

const portfolioPerks = [
  { icon: '📄', title: 'Официальное портфолио', desc: 'PDF с печатью организации — для ВУЗа, работы и грантов' },
  { icon: '⭐', title: 'Ранги и значки', desc: 'Новичок → Доброволец → Активист → Ветеран → Легенда' },
  { icon: '🔗', title: 'Поделиться в WhatsApp', desc: 'Один клик — и твоё портфолио у HR или приёмной комиссии' },
  { icon: '📊', title: 'Вся история', desc: 'Каждое мероприятие, каждый проект — в едином документе' },
]
    ...
    <div className="space-y-10">
      ...
      <section className="bg-white rounded-3xl card-shadow p-8 md:p-12 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
            ...
            {!user ? (
              <>
                ...
                <h1 className="font-brand text-4xl md:text-5xl text-[var(--color-text-heading)] mb-4 leading-tight">
                  ДЕЛАЙ ДОБРО.<br />
                  <span className="text-[var(--color-primary)]">СТРОЙ ПОРТФОЛИО.</span>
                </h1>
                <p className="text-[var(--color-text-body)] text-lg mb-8 max-w-xl leading-relaxed">
                  Каждое мероприятие — это строчка в твоём портфолио. 
                  Растёт ранг. Растёт опыт. Растут возможности.
                </p>
              ...
              <>
                <div className="inline-flex items-center gap-2 bg-indigo-50 text-indigo-600 rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider mb-6">
                  👋 С возвращением, {profile?.first_name || 'Волонтёр'}!
                </div>
                <h1 className="font-brand text-4xl md:text-5xl text-[var(--color-text-heading)] mb-4">
                  ГОТОВ К НОВЫМ <br />
                  <span className="text-[var(--color-primary)]">СВЕРШЕНИЯМ?</span>
                </h1>
                <p className="text-[var(--color-text-body)] text-lg mb-8 max-w-xl">
                  Твой текущий ранг: <span className="font-bold text-gray-800">{getRankName(profile?.total_hours || 0)}</span>. 
                  Ниже список горячих мероприятий, где нужны твои руки и сердце!
                </p>
          ...
          <div className="flex flex-col items-center group">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">Мяу! Это маскот Jas Volunteers 🐈</p>
          </div>
        </div>
      </section>
      ...
      {!user && (
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((s) => (
            <div key={s.label} className="bg-white rounded-2xl card-shadow p-5 text-center card-hover">
              <div className="text-3xl mb-2">{s.icon}</div>
              <div className="text-2xl font-bold text-[var(--color-text-heading)]">{s.value}</div>
              <div className="text-sm text-[var(--color-text-body)]">{s.label}</div>
            </div>
          ))}
        </section>
      )}
        ...
        <section className="bg-white rounded-2xl card-shadow p-8">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold text-[var(--color-text-heading)] mb-2">
              Твоё волонтёрство — это актив 📈
            </h2>
            <p className="text-[var(--color-text-body)] max-w-xl mx-auto">
              Мы превращаем добрые поступки в документально подтверждённый опыт
            </p>
          </div>
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4">
            {portfolioPerks.map((p) => (
              <div key={p.title} className="bg-[var(--color-surface)] rounded-2xl p-5 card-hover">
                <div className="text-3xl mb-3">{p.icon}</div>
                <h3 className="font-semibold text-[var(--color-text-heading)] mb-1 text-sm">{p.title}</h3>
                <p className="text-xs text-[var(--color-text-body)]">{p.desc}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <section className="bg-white rounded-3xl card-shadow p-8 border-b-4 border-red-50">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-brand text-[var(--color-text-heading)]">Система рангов</h2>
          <div className="text-xs font-bold uppercase tracking-widest text-gray-400">Путь волонтёра</div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          {[
            { rank: 'Новичок', desc: '0–49 ч', color: 'bg-gray-50 border-gray-100 text-gray-500', icon: <RankNovice className="w-6 h-6 mb-2" /> },
            { rank: 'Доброволец', desc: '50–199 ч', color: 'bg-emerald-50 border-emerald-100 text-emerald-700', icon: <RankVolunteer className="w-6 h-6 mb-2" /> },
            { rank: 'Активист', desc: '200–499 ч', color: 'bg-blue-50 border-blue-100 text-blue-700', icon: <RankActivist className="w-6 h-6 mb-2" /> },
            { rank: 'Ветеран', desc: '500–999 ч', color: 'bg-orange-50 border-orange-100 text-orange-700', icon: <RankVeteran className="w-6 h-6 mb-2" /> },
            { rank: 'Чемпион', desc: '1000–2499 ч', color: 'bg-purple-50 border-purple-100 text-purple-700', icon: <RankChampion className="w-6 h-6 mb-2" /> },
            { rank: 'Легенда', desc: '2500+ ч', color: 'bg-red-50 border-red-100 text-[var(--color-primary)]', icon: <RankLegend className="w-6 h-6 mb-2" /> },
          ].map((r) => (
            <div key={r.rank} className={`flex flex-col items-center rounded-2xl p-4 border transition-all hover:scale-105 ${r.color}`}>
              {r.icon}
              <div className="text-xs font-bold uppercase tracking-tight">{r.rank}</div>
      ...
      {displayEvents.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-[var(--color-text-heading)]">📋 Ближайшие мероприятия</h2>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1 mr-3">
                          <div className="flex justify-between text-xs text-[var(--color-text-body)] mb-0.5">
                            <span>Записалось</span><span>{spots}/{maxSpots}</span>
                          </div>
